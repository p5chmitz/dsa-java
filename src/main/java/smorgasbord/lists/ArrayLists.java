...
public class ArrayLists {
    ...
    public static class DynamicList {
        ...
        public DynamicList(int capacity) {
            list = new ArrayList<Entry>(capacity);
        }
        ...
        public void add(Entry e) {
            ...
            // Adds score if there are either open entries or its higher than the last score on the list
            // First check must be <= for 0-sized lists, short-circuiting second check
            if (numEntries <= list.size() || newScore > list.get(numEntries - 1).getScore( )) {
                // Drops no scores and increases the length of the object array
                if (numEntries < list.size()) {
                    numEntries++;
                }

                // Shift scores to make room for the new entry
                int j = numEntries;
                while (j > 0 && list.get(j - 1).getScore( ) < newScore) {
                    // shift entry from j-1 to j and decrement j
        ...
        public Entry remove(int i) throws IndexOutOfBoundsException {
            if (i < 0 || i >= numEntries)
                throw new IndexOutOfBoundsException("Invalid index: " + i);
            Entry temp = list.get(i);                 // save the object to be removed
            for (int j = i; j < numEntries - 1; j++)  // count up from i (not down)
                list.set(j, list.get(j + 1));         // move one cell to the left
            list.set(numEntries - 1, null);           // null out the old last score
            numEntries--;
            return temp;                              // return the removed object
        }
        ...
        public void printList(int len) {
            for (int e = 0; e < len; e++) {
                if (e >= list.size()) {return;} // Prevents OOB errors
                if (list.get(e) != null) {
    ...
    public static void staticListDriver() {
        ...
        for (int index : indicesToRemove) {
            System.out.println("Removing score at index " + index);
            staticList.remove(index);
            staticList.printList(3);
        }
        staticList.printSize();
    }
    /** This driver contains the calling code for both the dynamic array based scoreboard list structure */
